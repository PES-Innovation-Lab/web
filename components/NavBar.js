      ...
      return (
        <Link href={nav.route}>
          <Button
            disableRipple
            disableFocusRipple
            className={`navButton${active === nav.title ? ' active' : ''}`}
          >
            {nav.title}
          </Button>
        </Link>
...
const handleSearch = (query, searchSettings) => {
  ...
  for (const ele of elements) {
    if (ele.textContent.search(query) === -1) ele.style.display = 'none';
    else ele.style.display = searchSettings.default;
  }
};
...
=======
  const navs = [
    ...
    {
      title: 'Publications',
      route: `${process.env.ASSET_PREFIX}/publications`,
    },
    { title: 'Hashcode', route: `${process.env.ASSET_PREFIX}/hashcode` },
    { title: 'Our History', route: `${process.env.ASSET_PREFIX}/about_us` },
    {
...
>>>>>>> 4075af66d7319787b25d1ae7b48576d73c00a87c
    ...
    >
      <Toolbar>
        ...
        <Link href={`${process.env.ASSET_PREFIX}/`}>
          <IconButton disableFocusRipple edge="start" className="logoButton">
            <img
              className="logo"
              src={`${process.env.ASSET_PREFIX}/images/mlab/mlab_logo_icon.png`}
            />
          </IconButton>
        </Link>

        {navs.map((nav) => makeMenu(nav, active, false))}
        ...
        {search ? (
          <div className={classes.search}>
            ...
            <InputBase
              ...
              onInput={(e) => {
                handleSearch(e.target.value, searchSettings);
              }}
            />
          </div>
        ) : null}
      ...
      >
        <List className="navDrawer">
          ...
          {navs.map((nav) => makeMenu(nav, active, true))}
        </List>
      </SwipeableDrawer>
    </AppBar>
  );
